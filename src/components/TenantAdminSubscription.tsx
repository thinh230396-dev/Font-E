  ...
  return <div className="space-y-5">
    ...
    {activeTab === 'overview' && <div className="space-y-5">
      <section className="grid overflow-hidden rounded-3xl bg-gradient-to-br from-[#151126] via-[#241742] to-[#43256e] text-white shadow-xl lg:grid-cols-[1.35fr_0.65fr]">
        ...
        <div className="border-t border-white/10 bg-white/[0.05] p-6 lg:border-l lg:border-t-0">
          ...
          <div className="mt-5 rounded-2xl border border-white/10 bg-white/[0.06] p-4"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10"><CreditCard className="h-5 w-5 text-violet-200" /></span><div><p className="text-[9px] font-black">{defaultPaymentMethod || 'Chưa thiết lập thanh toán'}</p><p className="mt-1 text-[8px] text-slate-400">{defaultPaymentMethod ? 'Phương thức thanh toán chính' : 'Cần thêm trước kỳ gia hạn'}</p></div></div></div>
    ...
    {activeTab === 'billing' && <section className="space-y-5">
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-violet-50/60 p-5 sm:p-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-2 text-[8px] font-black uppercase tracking-[0.16em] text-violet-600"><ShieldCheck className="h-4 w-4" />Trung tâm thanh toán bảo mật</div>
            <h2 className="mt-2 text-xl font-black tracking-tight text-slate-950">Thanh toán & hóa đơn</h2>
            <p className="mt-1 text-[9px] leading-5 text-slate-500">Theo dõi công nợ, phương thức thanh toán và chứng từ của {tenantName} tại một nơi.</p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            <button type="button" onClick={() => setShowBillingProfile(true)} className="flex h-10 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-[9px] font-black text-slate-600 shadow-sm"><Building2 className="h-4 w-4" />Hồ sơ xuất hóa đơn</button>
            <button type="button" onClick={() => setShowPayment(true)} className="flex h-10 items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 text-[9px] font-black text-white shadow-sm"><CreditCard className="h-4 w-4" />Quản lý thanh toán</button>
          </div>
        </div>
        ...
        <div className="grid gap-4 p-5 sm:p-6 xl:grid-cols-[1.25fr_0.75fr]">
          <article className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#171328] via-[#261743] to-[#5b2b91] p-5 text-white shadow-lg sm:p-6">
            ...
            <div className="relative flex h-full flex-col justify-between gap-8">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className={`rounded-full px-3 py-1 text-[7px] font-black ring-1 ${defaultPaymentMethod ? 'bg-emerald-400/15 text-emerald-200 ring-emerald-300/20' : 'bg-amber-400/15 text-amber-200 ring-amber-300/20'}`}>{defaultPaymentMethod ? 'ĐÃ THIẾT LẬP' : 'CẦN THIẾT LẬP'}</span>
              </div>
              <div>
                <p className="text-[8px] font-bold uppercase tracking-[0.12em] text-violet-200">Phương thức thanh toán chính</p>
                <p className="mt-2 text-lg font-black">{defaultPaymentMethod || 'Chưa có phương thức thanh toán'}</p>
                <p className="mt-2 text-[8px] leading-4 text-slate-300">{defaultPaymentMethod ? 'Được dùng cho gia hạn và các hóa đơn đăng ký tiếp theo.' : 'Thêm phương thức để tránh gián đoạn dịch vụ khi đến kỳ gia hạn.'}</p>
              </div>
              <div className="flex flex-col gap-3 border-t border-white/10 pt-4 sm:flex-row sm:items-end sm:justify-between">
                <div><p className="text-[7px] font-bold uppercase tracking-wide text-slate-400">Kỳ thanh toán tiếp theo</p><p className="mt-1 text-[10px] font-black">{nextPayableInvoice ? `${money(nextPayableInvoice.amount, nextPayableInvoice.currency || billingCurrency)} · ${date(nextPayableInvoice.dueDate)}` : `Gia hạn ${date(renewalDate)}`}</p></div>
                <button type="button" onClick={() => setShowPayment(true)} className="flex h-9 items-center justify-center gap-2 rounded-xl bg-white px-4 text-[8px] font-black text-slate-900">Cập nhật<ArrowUpRight className="h-3.5 w-3.5" /></button>
              </div>
            </div>
          </article>
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
            <article className={`rounded-2xl border p-5 ${overdueCount ? 'border-rose-200 bg-rose-50' : 'border-amber-200 bg-amber-50'}`}>
              <div className="flex items-center justify-between"><span className={`flex h-10 w-10 items-center justify-center rounded-xl ${overdueCount ? 'bg-rose-100 text-rose-600' : 'bg-amber-100 text-amber-600'}`}><Clock3 className="h-5 w-5" /></span><span className={`text-[8px] font-black ${overdueCount ? 'text-rose-600' : 'text-amber-700'}`}>{payableInvoices.length} HÓA ĐƠN</span></div>
              <p className="mt-4 text-[9px] font-bold text-slate-500">Cần thanh toán</p>
              <p className="mt-1 text-xl font-black text-slate-950">{money(payableTotal, billingCurrency)}</p>
              <p className={`mt-2 text-[8px] font-bold ${overdueCount ? 'text-rose-600' : 'text-amber-700'}`}>{overdueCount ? `${overdueCount} hóa đơn đã quá hạn` : payableInvoices.length ? 'Tất cả vẫn còn trong hạn' : 'Không có công nợ cần xử lý'}</p>
            </article>
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 p-5 sm:p-6">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div><div className="flex items-center gap-2"><ReceiptText className="h-5 w-5 text-violet-600" /><h2 className="text-base font-black text-slate-900">Danh sách hóa đơn</h2></div><p className="mt-1 text-[9px] text-slate-500">Hiển thị {filteredInvoices.length} / {invoices.length} chứng từ thanh toán.</p></div>
          ...
          ] as Array<[typeof invoiceStatus, string, number]>).map(([value, label, count]) => <button key={value} type="button" onClick={() => setInvoiceStatus(value)} className={`shrink-0 rounded-full px-3 py-1.5 text-[8px] font-black transition ${invoiceStatus === value ? 'bg-violet-600 text-white shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>{label} · {count}</button>)}</div>
        </div>

        {filteredInvoices.length ? <>
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full min-w-[920px] text-left">
              <thead><tr className="bg-slate-50 text-[7px] font-black uppercase tracking-[0.08em] text-slate-400"><th className="px-5 py-3.5">Hóa đơn</th><th className="px-4 py-3.5">Chu kỳ</th><th className="px-4 py-3.5">Phát hành / đến hạn</th><th className="px-4 py-3.5">Phương thức</th><th className="px-4 py-3.5">Tổng tiền</th><th className="px-4 py-3.5">Trạng thái</th><th className="px-5 py-3.5 text-right">Thao tác</th></tr></thead>
              <tbody className="divide-y divide-slate-100">{filteredInvoices.map((invoice) => <tr key={invoice.id} className="group text-[9px] text-slate-600 transition hover:bg-violet-50/30">
                <td className="px-5 py-4"><button type="button" onClick={() => setSelectedInvoice(invoice)} className="text-left"><p className="font-black text-slate-900 group-hover:text-violet-700">{invoice.invoiceCode || invoice.id}</p><p className="mt-1 text-[7px] text-slate-400">{invoice.planName || current.name} · {invoice.type === 'PLAN_CHANGE' ? 'Thay đổi gói' : invoice.type === 'MANUAL_ADJUSTMENT' ? 'Điều chỉnh' : 'Đăng ký'}</p></button></td>
                <td className="px-4 py-4"><p className="font-bold text-slate-700">{invoice.billingPeriod || invoice.servicePeriod || '—'}</p><p className="mt-1 text-[7px] text-slate-400">{invoice.billingCycle === 'yearly' ? 'Hằng năm' : invoice.billingCycle === 'monthly' ? 'Hằng tháng' : ''}</p></td>
                <td className="px-4 py-4"><p>{date(invoice.createdAt)}</p><p className={`mt-1 text-[7px] font-bold ${invoice.status === 'OVERDUE' ? 'text-rose-600' : 'text-slate-400'}`}>Hạn {date(invoice.dueDate)}</p></td>
          ...
          <div className="divide-y divide-slate-100 md:hidden">{filteredInvoices.map((invoice) => <button key={invoice.id} type="button" onClick={() => setSelectedInvoice(invoice)} className="block w-full p-5 text-left hover:bg-slate-50"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-[10px] font-black text-slate-900">{invoice.invoiceCode || invoice.id}</p><p className="mt-1 truncate text-[8px] text-slate-400">{invoice.planName || current.name} · {invoice.billingPeriod}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[7px] font-bold ring-1 ${invoiceMeta[invoice.status].tone}`}>{invoiceMeta[invoice.status].label}</span></div><div className="mt-4 flex items-end justify-between gap-3"><div><p className="text-[7px] uppercase tracking-wide text-slate-400">Đến hạn</p><p className={`mt-1 text-[8px] font-bold ${invoice.status === 'OVERDUE' ? 'text-rose-600' : 'text-slate-600'}`}>{date(invoice.dueDate)}</p></div><p className="text-sm font-black text-slate-900">{money(invoice.amount, invoice.currency || billingCurrency)}</p></div></button>)}</div>
        </> : <div className="py-16 text-center"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-100"><ReceiptText className="h-6 w-6 text-slate-400" /></span><p className="mt-4 text-[10px] font-black text-slate-700">Không tìm thấy hóa đơn phù hợp</p><p className="mt-1 text-[8px] text-slate-400">Thử thay đổi từ khóa hoặc trạng thái.</p>{(invoiceQuery || invoiceStatus !== 'ALL') && <button type="button" onClick={() => { setInvoiceQuery(''); setInvoiceStatus('ALL'); }} className="mt-4 rounded-xl bg-violet-50 px-4 py-2 text-[8px] font-black text-violet-700">Xóa bộ lọc</button>}</div>}
      </div>
    </section>}
    ...
    {selectedInvoice && <div className="fixed inset-0 z-[100] flex justify-end bg-slate-950/60 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`Chi tiết hóa đơn ${selectedInvoice.invoiceCode || selectedInvoice.id}`}>
      ...
      <section className="relative flex h-full w-full max-w-xl flex-col bg-white shadow-2xl">
        <div className="border-b border-slate-100 p-5 sm:p-6">
          <div className="flex items-start justify-between gap-4">
            <div><p className="text-[8px] font-black uppercase tracking-[0.14em] text-violet-600">Chi tiết hóa đơn</p><h2 className="mt-2 text-xl font-black text-slate-950">{selectedInvoice.invoiceCode || selectedInvoice.id}</h2><p className="mt-1 text-[9px] text-slate-500">{selectedInvoice.planName || current.name} · {selectedInvoice.billingPeriod}</p></div>
          ...
          <div className="mt-5 flex items-end justify-between gap-4 rounded-2xl bg-slate-950 p-5 text-white">
            <div><p className="text-[8px] text-slate-400">Tổng thanh toán</p><p className="mt-1 text-2xl font-black">{money(selectedInvoice.amount, selectedInvoice.currency || billingCurrency)}</p></div>
            <span className={`rounded-full px-3 py-1.5 text-[8px] font-black ring-1 ${invoiceMeta[selectedInvoice.status].tone}`}>{invoiceMeta[selectedInvoice.status].label}</span>
          </div>
        </div>
        <div className="flex-1 space-y-5 overflow-y-auto p-5 sm:p-6">
          ...
          ].map(([label, value]) => <div key={label} className="rounded-2xl bg-slate-50 p-4"><p className="text-[7px] font-bold uppercase tracking-wide text-slate-400">{label}</p><p className="mt-2 text-[9px] font-black text-slate-800">{value}</p></div>)}</div>

          <div className="rounded-2xl border border-slate-200 p-4">
            <div className="flex items-center gap-2"><Building2 className="h-4 w-4 text-blue-600" /><h3 className="text-[10px] font-black text-slate-800">Thông tin nhận hóa đơn</h3></div>
            <div className="mt-4 space-y-2 text-[8px] text-slate-500"><p><strong className="text-slate-700">{selectedInvoice.billingCompany || billingCompany}</strong></p><p>{selectedInvoice.billingEmail || billingEmail || 'Chưa cập nhật email nhận hóa đơn'}</p><p>{selectedInvoice.taxCode ? `MST ${selectedInvoice.taxCode}` : billingTaxCode ? `MST ${billingTaxCode}` : 'Chưa cập nhật mã số thuế'}</p><p>{selectedInvoice.billingAddress || billingAddress || 'Chưa cập nhật địa chỉ xuất hóa đơn'}</p></div>
          </div>

          <div className="rounded-2xl border border-slate-200 p-4">
            <h3 className="text-[10px] font-black text-slate-800">Chi tiết khoản phí</h3>
            <div className="mt-4 space-y-3">{selectedInvoice.lineItems?.length ? selectedInvoice.lineItems.map((item) => <div key={item.id} className="flex items-start justify-between gap-4 border-b border-slate-100 pb-3 last:border-0 last:pb-0"><div><p className="text-[8px] font-bold text-slate-700">{item.description}</p><p className="mt-1 text-[7px] text-slate-400">{item.quantity} × {money(item.unitPrice, selectedInvoice.currency || billingCurrency)}</p></div><p className="text-[8px] font-black text-slate-800">{money(item.amount, selectedInvoice.currency || billingCurrency)}</p></div>) : <div className="flex items-center justify-between"><div><p className="text-[8px] font-bold text-slate-700">{selectedInvoice.planName || current.name}</p><p className="mt-1 text-[7px] text-slate-400">{selectedInvoice.servicePeriod || selectedInvoice.billingPeriod}</p></div><p className="text-[8px] font-black text-slate-800">{money(selectedInvoice.amount, selectedInvoice.currency || billingCurrency)}</p></div>}</div>
          </div>
