...
export default function TenantAdminSanitation({ searchQuery, onSearchQueryChange, selectedBranch, tenantName = 'Lumière Nail Studio', roleLabel = 'Owner · Tenant Admin', accessMode = 'full', readOnlyReason, onNotify }: TenantAdminSanitationProps) {
  ...
  const renderArchiveToggle = (archivedCount: number) => <div className="flex h-10 items-center rounded-xl border border-slate-200 bg-slate-50 p-1">
    <button type="button" onClick={() => { setShowArchived(false); setOpenActionMenu(null); }} className={`h-8 border-0 px-3 text-[11px] font-black shadow-none ${!showArchived ? 'bg-white text-slate-800 shadow-sm' : 'bg-transparent text-slate-500'}`}>Đang hoạt động</button>
    <button type="button" onClick={() => { setShowArchived(true); setOpenActionMenu(null); }} className={`flex h-8 items-center gap-1.5 border-0 px-3 text-[11px] font-black shadow-none ${showArchived ? 'bg-white text-violet-700 shadow-sm' : 'bg-transparent text-slate-500'}`}><Archive className="h-3.5 w-3.5" />Đã lưu trữ ({archivedCount})</button>
  </div>;
  ...
  return <div className="space-y-5">
    ...
    <section className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div><div className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-[0.12em] text-violet-600"><ShieldCheck className="h-4 w-4" />Kiểm soát tuân thủ</div><h1 className="text-2xl font-black tracking-[-0.035em] text-slate-950 sm:text-3xl">Vệ sinh & an toàn</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Kiểm soát checklist, sự cố và hồ sơ tuân thủ của {tenantName} theo từng chi nhánh.</p></div><button type="button" onClick={() => { if (!requireManage()) return; setFormError(''); setIncidentFormOpen(true); }} disabled={!canManage} className="flex h-11 w-full min-w-0 items-center justify-center gap-2 whitespace-nowrap rounded-xl border border-violet-700 bg-violet-600 px-5 text-xs font-black text-white shadow-lg shadow-violet-200 transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto xl:min-w-[164px]"><Plus className="h-4 w-4 shrink-0" /><span>Ghi nhận sự cố</span></button></section>
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.04)]"><div className="flex gap-1 overflow-x-auto border-b border-slate-100 px-3 pt-3 sm:px-5">{tabs.map((item) => <button key={item.id} type="button" onClick={() => switchTab(item.id)} className={`h-10 shrink-0 rounded-b-none border-x-0 border-t-0 px-3 text-xs font-black shadow-none ${tab === item.id ? 'border-b-2 border-violet-600 bg-violet-50/60 text-violet-700' : 'border-b-2 border-transparent bg-white text-slate-500'}`}>{item.label}</button>)}</div><div className="p-4 sm:p-5">
                ...
                return (
                  <div key={checklist.id} className="relative">
                    ...
                    >
                      ...
                      <p className="mt-4 text-[10px] font-black uppercase tracking-wide text-violet-600">
      ...
      {tab === 'INCIDENTS' && <div>
        ...
        {filteredIncidents.length > 0 ? <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filteredIncidents.map((incident) => <div key={incident.id} className="relative">
          <button type="button" onClick={() => setSelectedIncident(incident)} className={`h-full w-full border border-slate-200 p-5 text-left shadow-sm hover:border-violet-200 hover:shadow-lg ${showArchived ? 'bg-slate-50/70' : 'bg-white'}`}><div className="flex items-start justify-between gap-2 pr-10"><span className={`flex h-11 w-11 items-center justify-center rounded-xl ${incident.severity === 'HIGH' ? 'bg-rose-50 text-rose-600' : incident.severity === 'MEDIUM' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}><ShieldAlert className="h-5 w-5" /></span><div className="flex flex-wrap justify-end gap-1"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${severityMeta[incident.severity].badge}`}>Mức {severityMeta[incident.severity].label}</span><span className={`rounded-full px-2 py-1 text-[10px] font-bold ring-1 ${incidentStatusMeta[incident.status].badge}`}>{incidentStatusMeta[incident.status].label}</span></div></div><p className="mt-4 text-[10px] font-black uppercase tracking-wide text-violet-600">{incident.id} · {branchName(incident.branch)}</p><h3 className="mt-1 text-base font-black text-slate-900">{incident.title}</h3><p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-500">{incident.description}</p><div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3 text-[10px] font-bold text-slate-500"><span>{incident.area}</span><span>Phụ trách {incident.owner}</span></div></button>
      ...
      {tab === 'COMPLIANCE' && <div>
        ...
        <div className="mt-5 overflow-visible rounded-2xl border border-slate-200">
          <div className="hidden grid-cols-[1.4fr_1fr_100px_100px_120px_40px] gap-3 rounded-t-2xl border-b border-slate-100 bg-slate-50 px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-400 lg:grid"><span>Hồ sơ</span><span>Phạm vi / phụ trách</span><span>Ngày cấp</span><span>Hết hạn</span><span>Trạng thái</span><span /></div>
          <div className="divide-y divide-slate-100">{filteredCertificates.length > 0 ? filteredCertificates.map((cert) => <div key={cert.id} className="relative">
            <button type="button" onClick={() => setNotice(`Đã mở tài liệu ${cert.document}.`)} className={`grid h-auto w-full gap-3 rounded-none border-0 px-4 py-4 pr-16 text-left shadow-none hover:bg-slate-50 lg:grid-cols-[1.4fr_1fr_100px_100px_120px_40px] lg:items-center lg:pr-4 ${showArchived ? 'bg-slate-50/70' : 'bg-white'}`}><div><p className="text-xs font-black text-slate-800">{cert.name}</p><p className="mt-1 text-[10px] text-slate-400">{cert.id} · {cert.type} · {cert.document}</p></div><div><p className="text-xs font-bold text-slate-600">{branchName(cert.branch)}</p><p className="mt-1 text-[10px] text-slate-400">{cert.owner}</p></div><p className="text-xs font-bold text-slate-600">{cert.issued}</p><p className="text-xs font-bold text-slate-600">{cert.expires}</p><span className={`w-fit rounded-full px-2 py-1 text-[10px] font-bold ring-1 ${certStatusMeta[cert.status].badge}`}>{certStatusMeta[cert.status].label}</span><span aria-hidden="true" /></button>
                ...
                {selectedChecklist.tasks.map((task, index) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className={`flex h-8 shrink-0 items-center gap-1.5 rounded-lg border px-2.5 text-[10px] font-black shadow-sm disabled:cursor-not-allowed disabled:opacity-50 ${
                        task.done
    ...
    {selectedBatch && <div className="fixed inset-0 z-[70] flex justify-end bg-slate-950/45 backdrop-blur-[2px]"><button type="button" aria-label="Đóng chi tiết lô" onClick={() => setSelectedBatch(null)} className="absolute inset-0 min-h-0 rounded-none border-0 bg-transparent p-0 shadow-none" /><aside className="relative flex h-full w-full max-w-[520px] flex-col bg-white shadow-2xl"><div className="flex items-start justify-between border-b border-slate-100 p-5 sm:p-6"><div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ring-1 ${batchStatusMeta[selectedBatch.status].badge}`}>{batchStatusMeta[selectedBatch.status].label}</span><h2 className="mt-3 text-xl font-black text-slate-900">{selectedBatch.id}</h2><p className="mt-1 text-xs text-slate-400">{selectedBatch.machine} · {branchName(selectedBatch.branch)}</p></div><button type="button" onClick={() => setSelectedBatch(null)} aria-label="Đóng" className="flex h-9 w-9 items-center justify-center border border-slate-200 bg-white p-0 text-slate-500 shadow-sm"><X className="h-4 w-4" /></button></div><div className="flex-1 overflow-y-auto p-5 sm:p-6"><section className={`rounded-2xl p-5 text-white ${selectedBatch.status === 'QUARANTINE' || selectedBatch.status === 'FAILED' ? 'bg-rose-950' : 'bg-slate-950'}`}><p className="text-[10px] uppercase tracking-wide text-slate-400">Thông số chu trình</p><div className="mt-4 grid grid-cols-3 gap-3"><div><Thermometer className="h-4 w-4 text-rose-300" /><p className="mt-2 text-xl font-black">{selectedBatch.temperature}°C</p><p className="mt-1 text-[10px] text-slate-400">Nhiệt độ</p></div><div><Gauge className="h-4 w-4 text-blue-300" /><p className="mt-2 text-xl font-black">{selectedBatch.pressure}</p><p className="mt-1 text-[10px] text-slate-400">Áp suất</p></div><div><PackageCheck className="h-4 w-4 text-emerald-300" /><p className="mt-2 text-xl font-black">{selectedBatch.items}</p><p className="mt-1 text-[10px] text-slate-400">Túi dụng cụ</p></div></div></section><div className="mt-5 grid grid-cols-2 gap-3">{[{ label: 'Chu trình', value: selectedBatch.cycle }, { label: 'Thời gian', value: `${selectedBatch.start} → ${selectedBatch.end}` }, { label: 'Chỉ thị hóa học', value: selectedBatch.indicator }, { label: 'Người vận hành', value: selectedBatch.operator }, { label: 'Người phát hành', value: selectedBatch.releasedBy }, { label: 'Chi nhánh', value: branchName(selectedBatch.branch) }].map((item) => <div key={item.label} className="rounded-xl border border-slate-200 p-3"><p className="text-[10px] text-slate-400">{item.label}</p><p className="mt-1 text-xs font-black leading-5 text-slate-700">{item.value}</p></div>)}</div><section className={`mt-5 rounded-2xl p-4 ${selectedBatch.status === 'QUARANTINE' ? 'bg-rose-50' : 'bg-violet-50'}`}><p className={`text-xs font-black ${selectedBatch.status === 'QUARANTINE' ? 'text-rose-800' : 'text-violet-800'}`}>Kết luận lô</p><p className={`mt-2 text-xs leading-5 ${selectedBatch.status === 'QUARANTINE' ? 'text-rose-700' : 'text-violet-700'}`}>{selectedBatch.note}</p></section></div><div className="border-t border-slate-100 bg-slate-50 p-4 sm:px-6"><div className="flex gap-2"><button type="button" onClick={() => setNotice(`Đã mở nhãn in cho lô ${selectedBatch.id}.`)} className="flex h-11 items-center gap-2 border border-slate-200 bg-white px-4 text-xs font-bold text-slate-600 shadow-sm"><Printer className="h-4 w-4" />In nhãn lô</button><button type="button" onClick={() => releaseBatch(selectedBatch)} disabled={!canManage || selectedBatch.status !== 'RUNNING'} className="flex h-11 flex-1 items-center justify-center gap-2 border border-violet-700 bg-violet-600 px-4 text-xs font-black text-white shadow-lg shadow-violet-200"><BadgeCheck className="h-4 w-4" />Duyệt & phát hành</button></div></div></aside></div>}
    {selectedIncident && <div className="fixed inset-0 z-[70] flex justify-end bg-slate-950/45 backdrop-blur-[2px]"><button type="button" aria-label="Đóng chi tiết sự cố" onClick={() => setSelectedIncident(null)} className="absolute inset-0 min-h-0 rounded-none border-0 bg-transparent p-0 shadow-none" /><aside className="relative flex h-full w-full max-w-[560px] flex-col bg-white shadow-2xl"><div className="flex items-start justify-between border-b border-slate-100 p-5 sm:p-6"><div><div className="flex items-center gap-2"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${severityMeta[selectedIncident.severity].badge}`}>Mức {severityMeta[selectedIncident.severity].label}</span><span className={`rounded-full px-2 py-1 text-[10px] font-bold ring-1 ${incidentStatusMeta[selectedIncident.status].badge}`}>{incidentStatusMeta[selectedIncident.status].label}</span></div><h2 className="mt-3 text-xl font-black text-slate-900">{selectedIncident.title}</h2><p className="mt-1 text-xs text-slate-400">{selectedIncident.id} · {selectedIncident.occurredAt}</p></div><button type="button" onClick={() => setSelectedIncident(null)} aria-label="Đóng" className="flex h-9 w-9 items-center justify-center border border-slate-200 bg-white p-0 text-slate-500 shadow-sm"><X className="h-4 w-4" /></button></div><div className="flex-1 overflow-y-auto p-5 sm:p-6"><div className="grid grid-cols-2 gap-3">{[{ label: 'Danh mục', value: selectedIncident.category }, { label: 'Khu vực', value: `${selectedIncident.area} · ${branchName(selectedIncident.branch)}` }, { label: 'Người báo cáo', value: selectedIncident.reporter }, { label: 'Người phụ trách', value: selectedIncident.owner }].map((item) => <div key={item.label} className="rounded-xl border border-slate-200 p-3"><p className="text-[10px] text-slate-400">{item.label}</p><p className="mt-1 text-xs font-black text-slate-700">{item.value}</p></div>)}</div><section className="mt-5 rounded-2xl bg-slate-950 p-5 text-white"><p className="text-[10px] uppercase tracking-wide text-rose-300">Mô tả sự cố</p><p className="mt-2 text-sm leading-6 text-white">{selectedIncident.description}</p></section><div className="mt-5 space-y-3">{[{ label: 'Hành động tức thời', value: selectedIncident.immediateAction, icon: ShieldAlert, tone: 'bg-amber-50 text-amber-800' }, { label: 'Nguyên nhân gốc', value: selectedIncident.rootCause, icon: Search, tone: 'bg-blue-50 text-blue-800' }, { label: 'Hành động khắc phục', value: selectedIncident.correctiveAction, icon: Wrench, tone: 'bg-emerald-50 text-emerald-800' }].map(({ label, value, icon: Icon, tone }) => <section key={label} className={`rounded-2xl p-4 ${tone}`}><div className="flex items-start gap-3"><Icon className="mt-0.5 h-4 w-4 shrink-0" /><div><p className="text-xs font-black">{label}</p><p className="mt-2 text-xs leading-5 opacity-90">{value}</p></div></div></section>)}</div><div className="mt-5 rounded-xl border border-slate-200 p-3"><p className="text-[10px] text-slate-400">Hạn hoàn tất</p><p className="mt-1 text-xs font-black text-slate-700">{selectedIncident.dueDate}</p></div></div><div className="border-t border-slate-100 bg-slate-50 p-4 sm:px-6"><button type="button" onClick={() => resolveIncident(selectedIncident)} disabled={!canManage || selectedIncident.status === 'RESOLVED'} className="flex h-11 w-full items-center justify-center gap-2 border border-violet-700 bg-violet-600 px-4 text-xs font-black text-white shadow-lg shadow-violet-200"><Check className="h-4 w-4" />Xác nhận đã khắc phục</button></div></aside></div>}
      ...
      <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm">
        ...
        >
          ...
          <div className="grid gap-4 p-5 sm:grid-cols-2 sm:p-6">
            ...
            <div className="flex items-start gap-2 rounded-xl border border-blue-100 bg-blue-50 p-3 text-[11px] leading-5 text-blue-700 sm:col-span-2">
