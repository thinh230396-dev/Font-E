    ...
    <div className="space-y-5">
      ...
      {activeTab === 'invoices' && (
        <section className="rounded-xl border border-brand-outline/40 bg-brand-surface shadow-sm">
          <div className="border-b border-brand-outline/35 p-4"><div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5"><div className="relative xl:col-span-2"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-brand-text-muted" /><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Tìm hóa đơn, tenant, email hoặc mã giao dịch..." className="form-control pl-9" /></div><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as 'ALL' | Invoice['status'])} className="form-control"><option value="ALL">Mọi trạng thái</option>{Object.entries(STATUS_CONFIG).map(([value, config]) => <option key={value} value={value}>{config.label}</option>)}</select><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as typeof typeFilter)} className="form-control"><option value="ALL">Mọi loại hóa đơn</option>{Object.entries(TYPE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select value={dateRange} onChange={(event) => setDateRange(event.target.value as DateRange)} className="form-control"><option value="7D">7 ngày qua</option><option value="30D">30 ngày qua</option><option value="90D">90 ngày qua</option><option value="ALL">Toàn bộ thời gian</option></select></div><div className="mt-3 flex items-center justify-between gap-3"><p className="text-[10px] text-brand-text-muted">Hiển thị <strong className="text-brand-text">{filteredInvoices.length}</strong> / {invoices.length} hóa đơn</p><button onClick={resetFilters} className="min-h-0 border-0 bg-transparent px-2 py-1 text-[10px] font-bold text-brand-primary shadow-none">Đặt lại bộ lọc</button></div></div>
