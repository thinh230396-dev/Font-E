...
export default function BillingAndInvoices({
  ...
  const summary = useMemo(() => {
    ...
    const grossCollected = paid.reduce((sum, invoice) => sum + toReportCurrency(invoice.amount), 0);
    const refunded = paid.reduce((sum, invoice) => sum + toReportCurrency(invoice.refundedAmount || 0), 0);
    ...
    const pending = pendingInvoices.reduce((sum, invoice) => sum + toReportCurrency(invoice.amount), 0);
    const overdue = overdueInvoices.reduce((sum, invoice) => sum + toReportCurrency(invoice.amount), 0);
    const collectible = collected + pending + overdue;
    const fees = paid.reduce((sum, invoice) => sum + toReportCurrency(invoice.processingFee || 0), 0);
    return {
      grossCollected,
      collected,
      refunded,
  ...
  const agingBuckets = useMemo(() => {
    ...
    const build = (min: number, max: number) => {
      ...
      return { count: items.length, amount: items.reduce((sum, invoice) => sum + toReportCurrency(invoice.amount), 0) };
    };
    return [
  ...
  const gatewaySummary = useMemo(() => {
    ...
    return (['BANK_TRANSFER', 'MOMO', 'VNPAY', 'STRIPE', 'MANUAL'] as const).map((gateway) => {
      ...
      return { gateway, count: items.length, amount: items.reduce((sum, invoice) => sum + toReportCurrency((invoice.amount - (invoice.refundedAmount || 0))), 0) };
