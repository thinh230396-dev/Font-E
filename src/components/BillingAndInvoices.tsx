...
export default function BillingAndInvoices({ invoices, onUpdateInvoiceStatus, onUpdateInvoice, onCreateInvoice, showConfirm, reportCurrency }: BillingAndInvoicesProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
          <div className="space-y-4 xl:col-span-2">
            <section className="overflow-hidden rounded-2xl border border-brand-outline/40 bg-brand-surface shadow-sm"><div className="flex flex-col gap-3 border-b border-brand-outline/35 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-sm font-extrabold text-brand-text">Dòng tiền theo trạng thái</h2><p className="mt-1 text-[10px] text-brand-text-muted">Giá trị đã thu, đang chờ và quá hạn trong kỳ báo cáo.</p></div><Badge className={summary.collectionRate >= 90 ? 'border-emerald-500/25 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'border-amber-500/25 bg-amber-500/10 text-amber-600 dark:text-amber-400'}>{summary.collectionRate >= 90 ? 'Dòng tiền ổn định' : 'Cần theo dõi'}</Badge></div><div className="p-5"><div className="flex h-3 overflow-hidden rounded-full bg-brand-surface-highest">{summary.collectible > 0 && <><div title="Đã thu" style={{ width: `${summary.collected / summary.collectible * 100}%` }} className="h-full bg-emerald-500" /><div title="Đang chờ" style={{ width: `${summary.pending / summary.collectible * 100}%` }} className="h-full bg-amber-400" /><div title="Quá hạn" style={{ width: `${summary.overdue / summary.collectible * 100}%` }} className="h-full bg-red-500" /></>}</div><div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3"><div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4"><div className="flex items-center gap-2 text-[10px] font-bold text-emerald-600 dark:text-emerald-400"><span className="h-2 w-2 rounded-full bg-emerald-500" />Đã thu</div><p className="mt-2 font-mono text-sm font-extrabold text-brand-text">{formatMoney(summary.collected, reportCurrency)}</p><p className="mt-1 text-[9px] text-brand-text-muted">{summary.paidCount} hóa đơn</p></div><div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4"><div className="flex items-center gap-2 text-[10px] font-bold text-amber-600 dark:text-amber-400"><span className="h-2 w-2 rounded-full bg-amber-400" />Đang chờ</div><p className="mt-2 font-mono text-sm font-extrabold text-brand-text">{formatMoney(summary.pending, reportCurrency)}</p><p className="mt-1 text-[9px] text-brand-text-muted">{summary.pendingCount} hóa đơn</p></div><div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4"><div className="flex items-center gap-2 text-[10px] font-bold text-red-600 dark:text-red-400"><span className="h-2 w-2 rounded-full bg-red-500" />Quá hạn</div><p className="mt-2 font-mono text-sm font-extrabold text-brand-text">{formatMoney(summary.overdue, reportCurrency)}</p><p className="mt-1 text-[9px] text-brand-text-muted">{summary.overdueCount} hóa đơn</p></div></div></div></section>
      ...
      {selectedInvoice && (
        <div className="sa-modal-backdrop fixed inset-0 z-[70] flex items-center justify-center overscroll-contain bg-slate-950/60 p-3 backdrop-blur-[2px] sm:p-6" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedInvoiceId(null); }}>
