...
export default function DataBackup({ showConfirm }: DataBackupProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {selectedSnapshot && (
        <div className="fixed inset-0 z-[70] flex justify-end bg-brand-bg/70 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedSnapshotId(null); }}>
          <aside className="flex h-full w-full max-w-2xl flex-col border-l border-brand-outline bg-brand-surface shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-brand-outline/40 px-5 py-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-[11px] font-bold text-brand-primary">{selectedSnapshot.id}</span><Badge className={STATUS_CONFIG[selectedSnapshot.status].className}>{STATUS_CONFIG[selectedSnapshot.status].icon}{STATUS_CONFIG[selectedSnapshot.status].label}</Badge>{isImmutable(selectedSnapshot) && <Badge className="border-violet-500/25 bg-violet-500/10 text-violet-600 dark:text-violet-400"><LockKeyhole className="h-3 w-3" />Bất biến</Badge>}</div><h2 className="mt-2 truncate text-sm font-extrabold text-brand-text">{selectedSnapshot.filename}</h2><p className="mt-1 text-[10px] text-brand-text-muted">{TYPE_LABELS[selectedSnapshot.type]} · {SCOPE_LABELS[selectedSnapshot.scope]} · {formatDateTime(selectedSnapshot.createdAt)}</p></div><button onClick={() => setSelectedSnapshotId(null)} aria-label="Đóng chi tiết snapshot"><X className="h-5 w-5" /></button></div>
            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
              ...
              <div className="grid grid-cols-2 gap-3"><div className="rounded-xl border border-brand-outline/35 bg-brand-surface-high/35 p-3"><p className="text-[9px] font-bold uppercase text-brand-text-muted">Dung lượng</p><p className="mt-2 text-lg font-extrabold text-brand-text">{selectedSnapshot.size}</p><p className="mt-1 text-[9px] text-brand-text-muted">Nén {policy.compression}</p></div><div className="rounded-xl border border-brand-outline/35 bg-brand-surface-high/35 p-3"><p className="text-[9px] font-bold uppercase text-brand-text-muted">Thời gian chạy</p><p className="mt-2 text-lg font-extrabold text-brand-text">{formatDuration(selectedSnapshot.durationSeconds)}</p><p className="mt-1 text-[9px] text-brand-text-muted">Hoàn tất {formatDateTime(selectedSnapshot.completedAt)}</p></div></div>
              <section className="rounded-xl border border-brand-outline/35 p-4"><div className="mb-3 flex items-center gap-2"><Layers3 className="h-4 w-4 text-brand-primary" /><h3 className="text-xs font-extrabold text-brand-text">Thành phần snapshot</h3></div><div className="divide-y divide-brand-outline/25">{selectedSnapshot.components.map((component) => <div key={component.key} className="flex items-center justify-between gap-3 py-2.5"><div><p className="text-[10px] font-bold text-brand-text">{component.label}</p><p className="mt-0.5 text-[9px] text-brand-text-muted">{component.records !== undefined ? `${component.records.toLocaleString('vi-VN')} bản ghi` : component.status === 'SKIPPED' ? 'Không thuộc phạm vi backup' : 'Không thể hoàn tất'}</p></div><div className="text-right"><p className={`text-[10px] font-bold ${component.status === 'INCLUDED' ? 'text-emerald-600 dark:text-emerald-400' : component.status === 'FAILED' ? 'text-red-600 dark:text-red-400' : 'text-brand-text-muted'}`}>{component.status === 'INCLUDED' ? 'Đã bao gồm' : component.status === 'FAILED' ? 'Thất bại' : 'Bỏ qua'}</p><p className="mt-0.5 text-[9px] text-brand-text-muted">{component.size}</p></div></div>)}</div></section>
              <section className="rounded-xl border border-brand-outline/35 p-4"><div className="mb-3 flex items-center gap-2"><FileCheck2 className="h-4 w-4 text-brand-primary" /><h3 className="text-xs font-extrabold text-brand-text">Tính toàn vẹn & mã hóa</h3></div><dl className="space-y-3 text-[10px]"><div><dt className="text-brand-text-muted">Checksum SHA-256</dt><dd className="mt-1 break-all font-mono font-bold text-brand-text">{selectedSnapshot.checksum}</dd></div><div className="grid grid-cols-2 gap-3"><div><dt className="text-brand-text-muted">Trạng thái</dt><dd className={`mt-1 font-bold ${INTEGRITY_CONFIG[selectedSnapshot.integrityStatus].className}`}>{INTEGRITY_CONFIG[selectedSnapshot.integrityStatus].label}</dd></div><div><dt className="text-brand-text-muted">Xác minh lúc</dt><dd className="mt-1 font-bold text-brand-text">{formatDateTime(selectedSnapshot.verifiedAt)}</dd></div></div><div><dt className="text-brand-text-muted">Khóa mã hóa Cloud KMS</dt><dd className="mt-1 break-all font-mono font-bold text-brand-text">{selectedSnapshot.kmsKeyId}</dd></div></dl></section>
              <section className="rounded-xl border border-brand-outline/35 p-4"><div className="mb-3 flex items-center gap-2"><Cloud className="h-4 w-4 text-brand-primary" /><h3 className="text-xs font-extrabold text-brand-text">Lưu trữ & retention</h3></div><dl className="grid grid-cols-1 gap-3 text-[10px] sm:grid-cols-2"><div><dt className="text-brand-text-muted">Bucket chính</dt><dd className="mt-1 font-bold text-brand-text">gs://{selectedSnapshot.bucket}</dd></div><div><dt className="text-brand-text-muted">Vùng chính</dt><dd className="mt-1 font-bold text-brand-text">{selectedSnapshot.region}</dd></div><div><dt className="text-brand-text-muted">Bản sao dự phòng</dt><dd className="mt-1 font-bold text-brand-text">{selectedSnapshot.replicaRegion || 'Không tạo'}</dd></div><div><dt className="text-brand-text-muted">Retention class</dt><dd className="mt-1 font-bold text-brand-text">{RETENTION_LABELS[selectedSnapshot.retentionClass]}</dd></div><div><dt className="text-brand-text-muted">Hết hạn</dt><dd className="mt-1 font-bold text-brand-text">{formatDateTime(selectedSnapshot.expiresAt)}</dd></div><div><dt className="text-brand-text-muted">Khóa bất biến đến</dt><dd className="mt-1 font-bold text-brand-text">{formatDateTime(selectedSnapshot.immutableUntil)}</dd></div></dl></section>
              <section className="rounded-xl border border-brand-outline/35 p-4"><p className="text-[9px] font-bold uppercase text-brand-text-muted">Khởi tạo bởi</p><p className="mt-1 text-xs font-bold text-brand-text">{selectedSnapshot.initiatedBy}</p><p className="mt-3 text-[9px] font-bold uppercase text-brand-text-muted">Lý do / ghi chú</p><p className="mt-1 text-[10px] leading-relaxed text-brand-text">{selectedSnapshot.note || 'Không có ghi chú.'}</p></section>
            </div>
            <div className="flex flex-wrap justify-end gap-2 border-t border-brand-outline/40 bg-brand-surface-lowest/60 p-4"><button onClick={() => downloadManifest(selectedSnapshot)} className="inline-flex items-center gap-2 whitespace-nowrap border border-brand-outline bg-brand-surface px-3 py-2 text-xs font-bold text-brand-text"><FileJson className="h-4 w-4" /><span>Tải manifest</span></button>{selectedSnapshot.status === 'SUCCESS' && <button onClick={() => verifySnapshot(selectedSnapshot)} disabled={selectedSnapshot.integrityStatus === 'PENDING'} className="inline-flex items-center gap-2 whitespace-nowrap border border-brand-outline bg-brand-surface px-3 py-2 text-xs font-bold text-brand-text"><RefreshCw className={`h-4 w-4 ${selectedSnapshot.integrityStatus === 'PENDING' ? 'animate-spin' : ''}`} /><span>Xác minh lại</span></button>}{selectedSnapshot.status === 'SUCCESS' && selectedSnapshot.integrityStatus === 'VERIFIED' && <button onClick={() => openRestore(selectedSnapshot)} className="inline-flex items-center gap-2 whitespace-nowrap border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs font-bold text-amber-700 dark:text-amber-400"><RotateCcw className="h-4 w-4" /><span>Phục hồi</span></button>}<button onClick={() => deleteSnapshot(selectedSnapshot)} disabled={isImmutable(selectedSnapshot) || selectedSnapshot.status === 'IN_PROGRESS'} title={isImmutable(selectedSnapshot) ? `Đã khóa đến ${formatDateTime(selectedSnapshot.immutableUntil)}` : 'Xóa snapshot'} className="inline-flex items-center gap-2 whitespace-nowrap border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs font-bold text-red-700 dark:text-red-400"><Trash2 className="h-4 w-4" /><span>Xóa</span></button></div>
          </aside>
        </div>
      )}
