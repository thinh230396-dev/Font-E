      ...
      <div className="min-h-screen lg:pl-[248px]">
        <header className="sticky top-0 z-30 flex h-[76px] items-center gap-4 border-b border-slate-200/80 bg-white/90 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
          ...
          <div className="relative max-w-md flex-1">
            ...
            <input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder={activePage === 'Lịch hẹn' ? 'Tìm mã lịch, khách hàng, số điện thoại...' : activePage === 'Khách hàng' ? 'Tìm tên, mã khách, số điện thoại...' : activePage === 'Nhân sự' ? 'Tìm tên, mã nhân viên, kỹ năng...' : activePage === 'Dịch vụ & giá' ? 'Tìm tên, mã hoặc nhóm dịch vụ...' : 'Tìm khách hàng, dịch vụ, nhân viên...'} className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 text-[11px] font-medium outline-none transition focus:border-violet-400 focus:bg-white focus:ring-4 focus:ring-violet-100" />
          </div>
        ...
        <main className="mx-auto w-full max-w-[1440px] p-4 sm:p-6 lg:p-8">
          <Suspense fallback={<div className="flex min-h-[420px] items-center justify-center rounded-2xl border border-slate-200 bg-white"><div className="text-center"><span className="mx-auto block h-8 w-8 animate-spin rounded-full border-2 border-violet-200 border-t-violet-600" /><p className="mt-3 text-[9px] font-bold text-slate-400">Đang tải không gian quản lý...</p></div></div>}>
          {activePage === 'Lịch hẹn' ? (
