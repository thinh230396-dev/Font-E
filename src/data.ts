import { Tenant, SubscriptionPackage, SystemAlert, Invoice, SystemLog, BackupSnapshot, AdminSession, BackupPolicy, RestoreJob } from './types';
...
export const INITIAL_LOGS: SystemLog[] = [
  {
    ...
    eventCode: 'AUTH.LOGIN.SUCCESS',
    event: 'Đăng nhập Superadmin thành công',
    description: 'Tài khoản Superadmin đã hoàn tất xác thực mật khẩu và MFA.',
    ...
    resourceId: 'SES-CURRENT-001',
    location: 'TP. Hồ Chí Minh, VN',
    device: 'Chrome 136 · Windows 11',
    sessionId: 'SES-CURRENT-001',
    requestId: 'REQ-7F31A2D9',
    method: 'POST /auth/sessions',
    metadata: { mfa: true, authenticationMethod: 'password+totp' }
  },
  {
    id: 'AUD-20260715-002',
    timestamp: minutesAgo(32),
    eventCode: 'SECURITY.LOGIN.BLOCKED',
    event: 'Chặn đăng nhập sau nhiều lần thất bại',
    description: 'Hệ thống chặn tạm thời yêu cầu đăng nhập sau khi vượt giới hạn cho phép.',
    user: 'unknown',
    actorRole: 'SYSTEM',
    ip: '185.220.101.14',
    severity: 'high',
    status: 'blocked',
    category: 'SECURITY',
    resource: 'Cổng xác thực Superadmin',
    location: 'Frankfurt, DE',
    device: 'Firefox 128 · Linux',
    requestId: 'REQ-88CA10F1',
    method: 'POST /auth/sessions',
    metadata: { failedAttempts: 5, rule: 'MAX_LOGIN_ATTEMPTS' }
  },
  {
    id: 'AUD-20260715-003',
    timestamp: minutesAgo(86),
    eventCode: 'TENANT.STATUS.UPDATED',
    event: 'Cập nhật trạng thái tenant',
    description: 'Superadmin tạm ngưng tenant do yêu cầu kiểm tra thanh toán.',
    user: 'superadmin@salonsys.vn',
    actorRole: 'SUPERADMIN',
    ip: '113.161.72.18',
    severity: 'medium',
    status: 'success',
    category: 'TENANT',
    resource: 'Tenant Aurora Beauty',
    resourceId: 'TEN-AURORA',
    location: 'TP. Hồ Chí Minh, VN',
    device: 'Chrome 136 · Windows 11',
    sessionId: 'SES-CURRENT-001',
    requestId: 'REQ-57BC9210',
    method: 'PATCH /tenants/TEN-AURORA',
    changes: [{ field: 'status', before: 'ACTIVE', after: 'SUSPENDED' }]
  },
  {
    id: 'AUD-20260715-004',
    timestamp: minutesAgo(138),
    eventCode: 'BILLING.INVOICE.UPDATED',
    event: 'Đối soát trạng thái hóa đơn',
    description: 'Hóa đơn được xác nhận đã thanh toán sau khi đối chiếu giao dịch.',
    user: 'superadmin@salonsys.vn',
    actorRole: 'SUPERADMIN',
    ip: '113.161.72.18',
    severity: 'medium',
    status: 'success',
    category: 'BILLING',
    resource: 'Hóa đơn INV-2026-908',
    resourceId: 'INV-2026-908',
    location: 'TP. Hồ Chí Minh, VN',
    device: 'Chrome 136 · Windows 11',
    sessionId: 'SES-CURRENT-001',
    requestId: 'REQ-A0E9CB77',
    method: 'PATCH /invoices/INV-2026-908',
    changes: [{ field: 'status', before: 'PENDING', after: 'PAID' }]
  },
  {
    id: 'AUD-20260715-005',
    timestamp: minutesAgo(244),
    eventCode: 'USER.ADMIN.INVITED',
    event: 'Mời Tenant Admin mới',
    description: 'Một tài khoản Tenant Admin được gửi lời mời kích hoạt.',
    user: 'superadmin@salonsys.vn',
    actorRole: 'SUPERADMIN',
    ip: '113.161.72.18',
    severity: 'low',
    status: 'success',
    category: 'USER',
    resource: 'Tenant Admin linh.nguyen@example.vn',
    resourceId: 'ADM-1042',
    location: 'TP. Hồ Chí Minh, VN',
    device: 'Chrome 136 · Windows 11',
    sessionId: 'SES-CURRENT-001',
    requestId: 'REQ-002C4F12',
    method: 'POST /tenant-admins'
  },
  {
    id: 'AUD-20260715-006',
    timestamp: minutesAgo(390),
    eventCode: 'PACKAGE.PRICE.UPDATED',
    event: 'Thay đổi giá gói Premium',
    description: 'Tạo phiên bản giá mới; giá của tenant hiện hữu vẫn được khóa theo hợp đồng.',
    user: 'superadmin@salonsys.vn',
    actorRole: 'SUPERADMIN',
    ip: '113.161.72.18',
    severity: 'medium',
    status: 'success',
    category: 'PACKAGE',
    resource: 'Gói Premium',
    resourceId: 'PKG-2',
    location: 'TP. Hồ Chí Minh, VN',
    device: 'Chrome 136 · Windows 11',
    sessionId: 'SES-CURRENT-001',
    requestId: 'REQ-F311B208',
    method: 'PATCH /packages/PKG-2',
    changes: [
      { field: 'monthlyPrice', before: '89 USD', after: '99 USD' },
  ...
  },
  {
    ...
    eventCode: 'DATA.EXPORT.SUCCESS',
    event: 'Xuất báo cáo doanh thu hệ thống',
    description: 'Superadmin tải báo cáo tổng hợp doanh thu của toàn bộ tenant.',
    user: 'superadmin@salonsys.vn',
    actorRole: 'SUPERADMIN',
    ip: '14.161.22.91',
    severity: 'medium',
    status: 'success',
    category: 'DATA',
    resource: 'Báo cáo doanh thu tháng 07/2026',
    location: 'Đà Nẵng, VN',
    device: 'Safari 18 · macOS',
    sessionId: 'SES-REMOTE-002',
    requestId: 'REQ-94DB139F',
    method: 'GET /reports/revenue/export',
    metadata: { format: 'xlsx', rowCount: 148 }
  },
  {
    id: 'AUD-20260715-008',
    timestamp: minutesAgo(780),
    eventCode: 'AUTH.MFA.FAILED',
    event: 'Xác thực MFA thất bại',
    description: 'Mã TOTP không hợp lệ; yêu cầu đăng nhập bị từ chối.',
    user: 'superadmin@salonsys.vn',
    actorRole: 'SUPERADMIN',
    ip: '45.77.21.180',
    severity: 'high',
    ...
    requestId: 'REQ-2901ACD2',
    method: 'POST /auth/mfa/verify',
    metadata: { failedAttempts: 2, challengeExpired: false }
  },
  {
    id: 'AUD-20260715-009',
    timestamp: minutesAgo(1440),
    eventCode: 'SYSTEM.SETTINGS.UPDATED',
    event: 'Cập nhật chính sách bảo mật',
    description: 'Thay đổi thời gian hết hạn phiên quản trị.',
    user: 'superadmin@salonsys.vn',
    actorRole: 'SUPERADMIN',
    ip: '113.161.72.18',
    severity: 'medium',
    status: 'success',
    category: 'SYSTEM',
    resource: 'Cấu hình bảo mật',
    resourceId: 'SECURITY-POLICY',
    location: 'TP. Hồ Chí Minh, VN',
    device: 'Chrome 136 · Windows 11',
    sessionId: 'SES-CURRENT-001',
    requestId: 'REQ-00BF781C',
    method: 'PATCH /settings/security',
    changes: [{ field: 'sessionTimeout', before: '60 phút', after: '30 phút' }]
  }
];
